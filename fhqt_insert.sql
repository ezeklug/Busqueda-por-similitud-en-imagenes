create or replace function actualizar_arbol_insert() RETURNS TRIGGER AS
$$
DECLARE
vec record;
...
CREATE TRIGGER fhqt_insert
AFTER INSERT
ON imagenes
FOR EACH ROW
EXECUTE PROCEDURE actualizar_arbol_insert()



SELECT * FROM distancias

SELECT * from diez_vecinos_mas_cercanos((SELECT vector from imagenes limit 1),10)